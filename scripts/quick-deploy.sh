...
# ─── 步骤 5: 同步种子数据（幂等，每次部署执行）───
# 包含：管理员账号、会员价格、报告模板、兑换码
# 防止新环境/新表遗漏初始化数据（曾导致线上"暂无可用的报告模板"）
echo "[5/5] 同步种子数据（管理员/会员价格/报告模板）..."
sshpass -p "$PW" ssh "$SERVER" "cd $REMOTE && npx tsx prisma/seed.ts 2>&1 | tail -8"
echo "  ✓ 种子数据同步完成"
